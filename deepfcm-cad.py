...
class Particle:
    def __init__(self):
        ...
        df = pd.read_excel("suggested_weights_with_CNNoutput.xlsx", nrows=num_dimensions, engine='openpyxl')
        arr = df.to_numpy()
...
for train_index, test_index in kf.split(dataset):
    ...
    best_position,concept_evolution = minimize_and_plot_concept_evolution(training_dataset, num_dimensions=num_dimensions,  num_particles=40, maxiter=epoch)
    end = time.time()
    ...
    if(A!=100):
        ...
        df = pd.DataFrame(best_position, columns=["SEX","AGE", "BMI", "KNOWN CAD",
                                        "PREVIOUS AMI", "PREVIOUS PCI",
...
source_wb = load_workbook('suggested_weights_with_CNNoutput.xlsx')
